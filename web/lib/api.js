...
export const api = {
  ...
  transferencia: (id, contaDestinoId, valor, metodo, detalhe) =>
    req(`/contas/${id}/transferencia`, { method: "POST", body: JSON.stringify({ contaDestinoId, valor, metodo, detalhe }) }),
  simular: (body) => req("/emprestimos/simular", { method: "POST", body: JSON.stringify(body) }),
  analisarCredito: (body) => req("/credito/analisar", { method: "POST", body: JSON.stringify(body) }),
  ...
  cofrinhos: (contaId) => req(`/contas/${contaId}/cofrinhos`),
  criarCofrinho: (contaId, body) => req(`/contas/${contaId}/cofrinhos`, { method: "POST", body: JSON.stringify(body) }),
  guardarCofrinho: (id, valor) => req(`/cofrinhos/${id}/guardar`, { method: "POST", body: JSON.stringify({ valor }) }),
  resgatarCofrinho: (id, valor) => req(`/cofrinhos/${id}/resgatar`, { method: "POST", body: JSON.stringify({ valor }) }),
  perfil: () => req("/perfil"),
